...
function setUpTextures() {
  ...
  // load the actual image
  var customImage = document.getElementById('custom-texture');
  customImage.crossOrigin = "";
  customImage.onload = () => {
    ...
    // load the texture data
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, customImage.width, customImage.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, customImage);
    gl.bindTexture(gl.TEXTURE_2D, null);
    draw();
  }
}
...
function drawCurrentShape() {
  ...
  if (curTexture == "globe") {
    ...
    console.log("glob image");
    gl.uniform1f(program.uTextureType, 0.0);
    ...
    gl.uniform1f(program.uTextureType, 1.0);
    gl.activeTexture(gl.TEXTURE1);
    gl.bindTexture(gl.TEXTURE_2D, customTexture);    
    gl.uniform1i(program.uTheCustomTexture, 1);
    // console.log("custom texture", customTexture)
  } else if (curTexture == "proc") {
    
    // console.log("proc");
    gl.uniform1f(program.uTextureType, 2.0);
...
function initProgram(vertexid, fragmentid) {
  ...
  program.uTheCustomTexture = gl.getUniformLocation(program, 'theCustomTexture');
